...
import { useOnAutocompleteChange } from "./flux-autocomplete-field.hooks";
...
type FluxAutocompleteFieldProps = {
  name: string;
  options: Option[];
  multiple?: boolean;
  label: string;
...
export const FluxAutocompleteField = ({
  name,
  options,
  label,
  variant,
  multiple,
  helperText,
  "data-testid": dataTestId = "FluxAutocompleteField",
  ListboxProps,
  Icon,
  isOptionEqualToValue = (option, value) => option.id === value.id,
}: FluxAutocompleteFieldProps) => {
  const {
    field,
    fieldState: { error },
  } = useController({ name });
